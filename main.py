...
def startSignal():
	...
	db_conn = common.mongo_connect('history')
...
def finishSignal():
	...
	if not task:
		...
		return HTTPResponse(status=400,body=dumps({'error' : 'invalid request'}))
	if common.insert_finished_task(taskname=task,status_code=status,log=log_data,stime=start_time):
...
def hosts_of_project():
	...
	return common.hosts_of_project(project_name)
...
def addHostToProject():
	...
	if not project_name or not common.check_valid_ip(host) or not str(port).isdigit():
		return HTTPResponse(status=400,body=dumps({'error' : 'invalid request'}))
	...
	if common.add_host(project=project_name,ipaddr=host,port=port) :
...
def cronsOfProject():
	project_name = request.query.get('project')
	if not project_name : return HTTPResponse(status=400,body=dumps({'error' : 'invalid request'}))
	db_crons = common.mongo_connect('crons')
...
def update():
	...
	if not update:
		...
		for host in project_info['hosts']:
			...
			crontab = common.create_crontab(project=project_name,manager_url=config.get('manager','manager_url'),enabled=enable_flag)
			status = common.brodcast_crontab(host=ipaddr,port=port,user=project_info['user'],filename=crontab)
			if not status :
				failed_count += 1
...
def lastLog():
	...
	db_history = common.mongo_connect('history')
